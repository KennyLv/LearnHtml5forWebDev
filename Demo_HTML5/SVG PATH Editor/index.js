// forked from ynakajima's "SVG PATH Editor" http://jsdo.it/ynakajima/svgPathEditor
 /*!
 * SVGPathEditor 
...
(function(global){
    ...
    SVGPathEditor.prototype.init = function () {
		...
		this.editingRootLayer.addEventListener ("touchstart", function (e) {
			var _e = e.changedTouches[0];
		    	that.currentTouchEventId = _e.identifier;
			that._startDragHandler(_e);
		    	e.preventDefault();
		}, false);
		...
		this.editingRootLayer.addEventListener ("touchmove", function (e) {
			for (var i = 0, l = e.changedTouches.length; i < l; i++) {
			     if (e.changedTouches[i].identifier == that.currentTouchEventId) {
				that._dragHandler(e.changedTouches[i]);
			     }
			 }
			 e.preventDefault();
		}, false);
		...
		this.editingRootLayer.addEventListener ("touchend", function (e) {
			for (var i = 0, l = e.changedTouches.length; i < l; i++) {
			     if (e.changedTouches[i].identifier == that.currentTouchEventId) {
				that.currentTouchEventId = null;
				that._stopDragHandler(e.changedTouches[i]);
			     }
			 }
		}, false);
		this.editingRootLayer.addEventListener ("touchcancel", function (e) {
			for (var i = 0, l = e.changedTouches.length; i < l; i++) {
			     if (e.changedTouches[i].identifier == that.currentTouchEventId) {
				that.currentTouchEventId = null;
				that._stopDragHandler(e.changedTouches[i]);
