...
define(function(require, exports, module) {
	...
	(function($){
		$.fn.wx = function(option){
			...
			if($('#r-wx-title').val()!='') option.title = $('#r-wx-title').val();
			if($('#r-wx-img').val()!='') option.img = $('#r-wx-img').val();
			if($('#r-wx-con').val()!='') option.con = $('#r-wx-con').val();
			if($('#r-wx-link').val()!='') option.link = $('#r-wx-link').val();
			...
			var opt3 = {
				"bar" : opts.bar==undefined ? false : opts.bar,
				"menu" : opts.menu==undefined ? true : opts.menu
			};
			...
			handler = {
				callback : function(res){
					if(res.err_msg=="share_timeline:confirm"||res.err_msg=="share_timeline:ok"||res.err_msg=="send_app_msg:ok"||res.err_msg=="send_app_msg:confirm"){
						// 分享后跳转动作执行
						if(typeof window.shareGoto=="function"){
							window.shareGoto();
						}
						// 更新分享次数
						if(typeof utils.updateShareCount=="function"){
							utils.updateShareCount();
						}
			...
			_api.setOptionMenu(opt3.menu);
			_api.setToolbar(opt3.bar);

			return $wx;
		}
		$.fn.wx.defaults = {
			title : 'test', 
			con : 'test by hoperuncloud！！',
			link : document.URL, 
			img  : 'http://www.oupai168.com/app/img/logo.jpg'
		};
